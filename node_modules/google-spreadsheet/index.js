  ...
  {	
    ...
    if ( method == 'POST' || method == 'PUT' ){
      headers['content-type'] = 'application/atom+xml';
	  	//console.log('queryCALL:' + query_or_data);
    }
	...
	if (method == 'POST')
	{
		...
		//console.log('query:' + query_or_data);
	}	
		...
		request( {
		  ...
		  if ( body ){
			xml_parser.parseString(body, function(err, result){
			  if ( err ) return cb( err );
			  console.log('Tweet was logged successfully! ' + new Date().getJSON());
			  cb( null, result, body );
			});
		  } else {
			if ( err ) cb( err );
